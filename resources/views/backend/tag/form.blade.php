...
@section('title') Tag Form @endsection
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form class="row g-3 needs-validation" method="post" action="{{isset($tag)?route('tag.update', $tag->slug): route('tag.store')}}" novalidate>
                            @csrf
                            <div class="col-md-6">
                               <label for="name" class="form-label">Name of Tag</label>
                               <input type="text" class="form-control" id="name" name="name" placeholder="Tag name here.." value="{{isset($tag)? $tag->name: old('name')}}" required>
                               <div class="valid-feedback">
                                  Looks good!
                               </div>
                            </div>
                            <div class="col-6"><br>
                               <button class="btn btn-primary" type="submit">{{isset($tag)?'Update': 'Submit'}}</button>
                            </div>
                        </form>
                    </div>
                </div>
